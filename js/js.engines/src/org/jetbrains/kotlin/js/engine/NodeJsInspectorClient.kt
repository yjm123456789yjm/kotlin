...
private class NodeJsInspectorClientContextImpl(engine: NodeJsInspectorClient) : NodeJsInspectorClientContext, CDPRequestEvaluator {
    ...
    private val debugUrl: String = run {
        ...
        val bytesRead = nodeProcess.errorStream.read(buffer)
        val prompt = buffer.decodeToString(endIndex = bytesRead)
        prompt.lineSequence().firstNotNullOfOrNull {
            if (it.startsWith(NODE_WS_DEBUG_URL_PREFIX)) {
                val startIndexInLine = NODE_WS_DEBUG_URL_PREFIX.length - "ws://".length
                it.substring(startIndexInLine).trim()
            } else null
        } ?: error(prompt)
    }
