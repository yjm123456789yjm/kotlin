...
class KotlinBuilder : ModuleLevelBuilder(BuilderCategory.SOURCE_PROCESSOR) {
    ...
    private fun computeKotlinPathsForJpsPlugin(messageCollector: MessageCollectorAdapter): KotlinPaths? {
        ...
        return if (jpsKotlinHome?.exists() == true) {
            KotlinPathsFromHomeDir(jpsKotlinHome)
        } else {
            messageCollector.report(ERROR, "Cannot find kotlinc home at ($jpsKotlinHome). Make sure that '$JPS_KOTLIN_HOME_PROPERTY' system property is set.")
            null
        }
    }
