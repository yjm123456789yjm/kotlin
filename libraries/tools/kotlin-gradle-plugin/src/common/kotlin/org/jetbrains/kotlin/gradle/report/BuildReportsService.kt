...
abstract class BuildReportsService : BuildService<BuildReportsService.Parameters>, AutoCloseable, OperationCompletionListener {
    ...
    //To avoid java.lang.ClassNotFoundException: com.gradle.scan.plugin.BuildScanExtension
    abstract val buildScan: Property<BuildScanExtensionHolder?>
    ...
    private fun addBuildScanReport(event: FinishEvent?) {
        val buildScanSettings = parameters.reportingSettings.orNull?.buildScanReportSettings
        if (buildScanSettings != null && buildScan.orNull?.buildScan != null) {
            if (event is TaskFinishEvent) {
                ...
                compileStatData?.also {
                    addBuildScanReport(it, buildScanSettings.customValueLimit)
                }
            }
        }
    }

    private fun addBuildScanReport(data: CompileStatisticsData, customValuesLimit: Int) {
        val elapsedTime = measureTimeMillis {
            buildScan.orNull?.buildScan?.also { buildScan ->
    ...
    companion object {
        ...
        fun registerIfAbsent(project: Project, buildMetricsService: Provider<BuildMetricsService>?): Provider<BuildReportsService>? {
            if (buildMetricsService == null) {
                log.info("Unable to register BuildReportService without BuildMetricsService")
                return null
            }
            ...
            return gradle.sharedServices.registerIfAbsent(serviceName, serviceClass) {
                ...
                it.get().buildScan.set(buildScanExtension?.let { BuildScanExtensionHolder(it) })
                if (reportingSettings.httpReportSettings != null || (reportingSettings.buildScanReportSettings != null && buildScanExtension != null)
                ) {
