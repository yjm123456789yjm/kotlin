...
open class KotlinBrowserJs @Inject constructor(target: KotlinJsTarget) :
    ...
    private fun KotlinWebpack.commonConfigure(
        ...
        entryProperty.set(
            project.layout.file(
                compilation.compileKotlinTaskProvider
                    .flatMap { it.outputFileProperty }
                    .flatMap { outputFile ->
                        actualDceTaskProvider.flatMap {  dceTask ->
                            dceTask.destinationDirectory.file(outputFile.name).map { it.asFile }
                        }
                    }
            )
        )
    ...
    private fun configureDce(
        ...
        return project.registerTask(dceTaskName) {
            ...
            it.setSource(kotlinTask.flatMap { it.outputFileProperty })
        }
    }
