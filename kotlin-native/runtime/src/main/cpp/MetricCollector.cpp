...
public:
    ...
    void ZoneStart(std::string_view name) noexcept {
        ...
        if (!zoneStart) {
          ++missed_;
          RuntimeLogWarning({"metrics"}, "Failed to ZoneStart. Total missed: %llu", missed_);
          return;
        }
    ...
    void ZoneEnd(std::string_view name) noexcept {
        ...
        if (!zoneEnd) {
          ++missed_;
          RuntimeLogWarning({"metrics"}, "Failed to ZoneEnd. Total missed: %llu", missed_);
          return;
        }
    ...
    void Post(std::string_view name, int64_t value) noexcept {
        ...
        if (!signal) {
          ++missed_;
          RuntimeLogWarning({"metrics"}, "Failed to Signal. Total missed: %llu", missed_);
          return;
        }
