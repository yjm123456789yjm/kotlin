...
import org.jetbrains.kotlin.asJava.decompiler.stubBuilder.flags.*
...
private fun shouldSkip(flags: Int, name: Name): Boolean {
    return when (Flags.MEMBER_KIND.get(flags)) {
        ProtoBuf.MemberKind.FAKE_OVERRIDE, ProtoBuf.MemberKind.DELEGATION -> true
        //TODO: fix decompiler to use sane criteria
        ProtoBuf.MemberKind.SYNTHESIZED -> !DataClassDescriptorResolver.isComponentLike(name)
        else -> false
    }
}
...
private class FunctionClsStubBuilder(
    ...
    override fun doCreateCallableStub(parent: StubElement<out PsiElement>): StubElement<out PsiElement> {
        ...
        return KotlinFunctionStubImpl(
            ...
            hasBlockBody = true,
            hasBody = Flags.MODALITY.get(functionProto.flags) != ProtoBuf.Modality.ABSTRACT,
