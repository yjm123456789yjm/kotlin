...
import org.jetbrains.kotlin.asJava.decompiler.textBuilder.DeserializerForDecompilerBase
import org.jetbrains.kotlin.asJava.decompiler.textBuilder.LoggingErrorReporter
import org.jetbrains.kotlin.asJava.decompiler.textBuilder.ResolveEverythingToKotlinAnyLocalClassifierResolver
...
class DirectoryBasedClassFinder(
    ...
    override fun findKotlinClassOrContent(classId: ClassId): KotlinClassFinder.Result? {
        ...
        if (virtualFile != null && isKotlinWithCompatibleAbiVersion(virtualFile)) {
            return IDEKotlinBinaryClassCache.getInstance().getKotlinBinaryClass(virtualFile)?.let(KotlinClassFinder.Result::KotlinClass)
        }
        return null
    }
