...
package org.jetbrains.kotlin.asJava.decompiler.stubBuilder
...
class TypeClsStubBuilder(private val c: ClsStubBuilderContext) {
    ...
    private fun nullableTypeParent(parent: KotlinStubBaseImpl<*>, type: ProtoBuf.Type): KotlinStubBaseImpl<*> = if (type.nullable)
        KotlinPlaceHolderStubImpl<KtNullableType>(parent, KtStubElementTypes.NULLABLE_TYPE)
    else
        parent

    private fun createTypeParameterStub(parent: KotlinStubBaseImpl<*>, type: ProtoBuf.Type, name: Name, annotations: List<ClassIdWithTarget>) {
    ...
    private fun createClassReferenceTypeStub(parent: KotlinStubBaseImpl<*>, type: ProtoBuf.Type, annotations: List<ClassIdWithTarget>) {
        ...
        val shouldBuildAsFunctionType = isBuiltinFunctionClass(classId) && type.argumentList.none { it.projection == ProtoBuf.Type.Argument.Projection.STAR }
    ...
    private fun createTypeAnnotationStubs(parent: KotlinStubBaseImpl<*>, type: ProtoBuf.Type, annotations: List<ClassIdWithTarget>) {
    ...
    private fun getTypeModifiersAsWritten(type: ProtoBuf.Type): Set<KtModifierKeywordToken> {
    ...
    private fun ProtoBuf.Type.Argument.Projection.toProjectionKind() = when (this) {
        ProtoBuf.Type.Argument.Projection.IN -> KtProjectionKind.IN
        ProtoBuf.Type.Argument.Projection.OUT -> KtProjectionKind.OUT
        ProtoBuf.Type.Argument.Projection.INV -> KtProjectionKind.NONE
        ProtoBuf.Type.Argument.Projection.STAR -> KtProjectionKind.STAR
    }
    ...
    private fun createFunctionTypeStub(
        ...
        var suspendParameterType: ProtoBuf.Type? = null
    ...
    fun createTypeParameterListStub(
        ...
        val protosForTypeConstraintList = arrayListOf<Pair<Name, ProtoBuf.Type>>()
        for (proto in typeParameterProtoList) {
            ...
            val typeParameterStub = KotlinTypeParameterStubImpl(
                ...
                name = name.ref(),
                isInVariance = proto.variance == ProtoBuf.TypeParameter.Variance.IN,
                isOutVariance = proto.variance == ProtoBuf.TypeParameter.Variance.OUT
            )
    ...
    fun createTypeConstraintListStub(
        parent: StubElement<out PsiElement>,
        protosForTypeConstraintList: List<Pair<Name, ProtoBuf.Type>>
    ) {
    ...
    private fun createTypeParameterModifierListStub(
        ...
        when (typeParameterProto.variance) {
            ProtoBuf.TypeParameter.Variance.IN -> modifiers.add(KtTokens.IN_KEYWORD)
            ProtoBuf.TypeParameter.Variance.OUT -> modifiers.add(KtTokens.OUT_KEYWORD)
            ProtoBuf.TypeParameter.Variance.INV -> { /* do nothing */
            }
            null -> { /* do nothing */
            }
        }
    ...
    private fun ProtoBuf.Type.isDefaultUpperBound(): Boolean {
