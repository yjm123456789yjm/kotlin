...
fun checkUpperBoundViolated(
    ...
    val typeArgumentsCount = arguments.size
    ...
    val count = minOf(typeParameters.size, typeArgumentsCount)

    val typeSystemContext = context.session.typeContext

    for (index in 0 until count) {
        val argument = arguments.getOrNull(index) ?: continue
        val typeArgument: ConeKotlinType? = argument.coneTypeProjection.type
        val typeArgumentTypeRef = argument.typeRef
        val typeArgumentSource = argument.source
