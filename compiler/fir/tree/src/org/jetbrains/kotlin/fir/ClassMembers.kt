...
inline fun <reified D : FirCallableDeclaration> D.containsOverride(declaration: D): Boolean {
    var current = this

    do {
        val next = current.originalIfFakeOverride() ?: return false
        if (next === declaration) return true
        current = next
    } while (true)
}
