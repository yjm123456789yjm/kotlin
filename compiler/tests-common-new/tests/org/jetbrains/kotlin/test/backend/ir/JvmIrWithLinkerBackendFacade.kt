...
class JvmIrWithLinkerBackendFacade(
    testServices: TestServices
) : BackendFacade<KLibMiddleendOutput, BinaryArtifacts.Jvm>(testServices, MiddleendKinds.KLibIrBackend, ArtifactKinds.Jvm) {
    private val javaCompilerFacade = JavaCompilerFacade(testServices)

    override fun transform(
        module: TestModule,
        inputArtifact: KLibMiddleendOutput
    ): BinaryArtifacts.Jvm? {
        require(inputArtifact is KLibMiddleendOutput.JvmKLibIrMiddleendOutput) {
            "JvmIrWithLinkerBackendFacade expects KLibBackendOutput.JvmKLibIrBackendOutput as input"
        }
