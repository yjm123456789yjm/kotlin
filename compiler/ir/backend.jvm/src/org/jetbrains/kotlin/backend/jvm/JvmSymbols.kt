...
class JvmSymbols(
    ...
    val serializedLambda = createClass(FqName("java.lang.invoke.SerializedLambda")) { irClass ->
        irClass.addFunction("getImplMethodName", irBuiltIns.stringType)
        irClass.addFunction("getCapturedArg", irBuiltIns.anyNType).apply {
            addValueParameter(Name.identifier("i"), irBuiltIns.intType)
        }
    }

    val serializedLambdaGetImplMethodName = serializedLambda.functionByName("getImplMethodName")
    val serializedLambdaGetCapturedArg = serializedLambda.functionByName("getCapturedArg")
    ...
    // Compares SerializedLambda information with a known serialized method reference.
    // Assumes that `lambda.getImplMethodName()` is already checked.
    // Equivalent to (order of comparisons might be different):
    //      lambda.getImplClass() == IMPL_CLASS_INTERNAL_NAME &&
    //      lambda.getImplMethodKind() == IMPL_METHOD_KIND &&
    //      lambda.getImplMethodSignature() == IMPL_METHOD_SIGNATURE &&
    //      lambda.getFunctionalInterfaceClass() == SAM_CLASS_INTERNAL_NAME &&
    //      lambda.getFunctionalInterfaceMethodName() == SAM_METHOD_NAME &&
    //      lambda.getFunctionalInterfaceMethodSignature() == SAM_METHOD_SIGNATURE
    // Single intrinsic call is used to avoid additional calls to method signature mapper.
    val serializedLambdaEquals: IrSimpleFunctionSymbol =
        irFactory.buildFun {
            name = Name.special("<jvm-serialized-lambda-equals>")
            origin = IrDeclarationOrigin.IR_BUILTINS_STUB
        }.apply {
            returnType = irBuiltIns.booleanType
            parent = kotlinJvmInternalPackage
            addTypeParameter("<SAM_TYPE>", irBuiltIns.anyType)
            addValueParameter("lambda", serializedLambda.owner.defaultType)
            addValueParameter("samMethodType", irBuiltIns.anyNType)
            addValueParameter("implMethodRef", irBuiltIns.anyNType)
        }.symbol
