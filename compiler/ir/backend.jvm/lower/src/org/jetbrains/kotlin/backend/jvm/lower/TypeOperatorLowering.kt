...
private class TypeOperatorLowering(private val context: JvmBackendContext) : FileLoweringPass, IrBuildingTransformer(context) {
    ...
    private fun generateDeserializeLambdaMethod(
        ...
        //              when {
        //                  ...
        //                  `<jvm-serialized-lambda-equals>`(lambda, [INFO_i_k]) ->
        //                      `<jvm-indy>`([INFO_i_k])
        //                  ...
        //              }
        //          }
        ...
        val groupedByImplMethodName = serializableMethodRefInfos.groupBy {
            context.methodSignatureMapper.mapFunctionName(it.implFunSymbol.owner)
        }
        ...
        val lambdaParameter = deserializeLambdaFun.addValueParameter("lambda", context.ir.symbols.serializedLambda.defaultType)
        ...
        deserializeLambdaFun.body = context.createJvmIrBuilder(deserializeLambdaFun.symbol, UNDEFINED_OFFSET, UNDEFINED_OFFSET).run {
            irBlockBody {
                val tmp = irTemporary(
                    irCall(backendContext.ir.symbols.serializedLambdaGetImplMethodName).apply {
                    ...
                    groupedByImplMethodName.entries.map { (implMethodName, infos) ->
                        irBranch(
                            ...
                            irWhen(
                                ...
                                infos.map { info ->
                                    irBranch(
                                        generateSerializedLambdaEquals(lambdaParameter, info),
                                        irReturn(generateCreateDeserializedMethodRef(lambdaParameter, info))
                                    )
                                }
                            )
                        )
    ...
    private fun JvmIrBuilder.generateSerializedLambdaEquals(
        ...
        irCall(backendContext.ir.symbols.serializedLambdaEquals).also { call ->
            call.putTypeArgument(0, info.samType)
            call.putValueArgument(0, irGet(lambdaParameter))
            call.putValueArgument(1, irRawFunctionReferefence(backendContext.irBuiltIns.anyType, info.samMethodSymbol))
            call.putValueArgument(2, irRawFunctionReferefence(backendContext.irBuiltIns.anyType, info.implFunSymbol))
        }
    ...
    private fun JvmIrBuilder.generateCreateDeserializedMethodRef(
        ...
        for ((index, dynamicValueParameter) in info.dynamicCallSymbol.owner.valueParameters.withIndex()) {
            val capturedArg = irCall(backendContext.ir.symbols.serializedLambdaGetCapturedArg).also { call ->
    ...
    private fun rewriteIndyLambdaMetafactoryCall(call: IrCall): IrCall {
        ...
        val shouldBeSerializable = run {
            val shouldBeSerializableConst = call.getValueArgument(4) as? IrConst<*>
                ?: fail("'shouldBeSerializable' is expected to be 'IrConst'")
            shouldBeSerializableConst.value as? Boolean
                ?: fail("'shouldBeSerializable' is expected to be Boolean constant")
        }
