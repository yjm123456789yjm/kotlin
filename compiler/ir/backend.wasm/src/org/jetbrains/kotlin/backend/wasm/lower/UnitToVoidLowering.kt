...
class UnitToVoidLowering(val context: WasmBackendContext) : FileLoweringPass, AbstractValueUsageTransformer(context.irBuiltIns) {
    ...
    private fun voidify(expr: IrStatement): IrExpression {
        if (expr !is IrExpression) {
            // composite: Void {
            //   expr: variable declaration or something
            //   consumeAnyToVoid(null): Void
            // }
            return IrCompositeImpl(expr.startOffset, expr.endOffset, symbols.voidType, JsStatementOrigins.SYNTHESIZED_STATEMENT).apply {
                statements.add(expr)
                statements.add(IrCallImpl(expr.startOffset, expr.endOffset, symbols.voidType, symbols.consumeAnyIntoVoid, 0, 1).apply {
                    putValueArgument(0, IrConstImpl.constNull(startOffset, endOffset, builtIns.anyType))
                })
            }
        }
        ...
        when (expr) {
            ...
            is IrWhen -> {
                ...
                expr.branches.forEachIndexed { i, branch ->
                    expr.branches[i].result = voidify(branch.result)
                }
                return expr
            }
            is IrTry -> {
                expr.type = symbols.voidType
                expr.tryResult = voidify(expr.tryResult)
                expr.catches.forEachIndexed { i, catch ->
                    expr.catches[i].result = voidify(catch.result)
                }
                return expr
            }
            is IrTypeOperatorCall -> {
