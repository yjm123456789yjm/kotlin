...
abstract class PropertyDelegationLowering(open val context: CommonBackendContext) : FileLoweringPass {
    ...
    abstract fun IrField.posProcessKPropertiesField()

    fun interface KTypeGeneratorInterface {
        fun IrBuilderWithScope.irKType(type: IrType): IrExpression
    }

    abstract fun createKTypeGenerator(irFile: IrFile, expression: IrExpression): KTypeGeneratorInterface
    ...
    override fun lower(irFile: IrFile) {
        ...
        irFile.transformChildrenVoid(object : IrElementTransformerVoidWithContext() {
            ...
            override fun visitPropertyReference(expression: IrPropertyReference): IrExpression {
                ...
                val kTypeGenerator = createKTypeGenerator(irFile, expression)
                    ...
                    if (receiversCount == 2)
                        ...
                        val field = kProperties.getOrPut(expression.symbol.owner) {
                            createLocalKProperty(
                                ...
                                createKTypeGenerator(irFile, expression),
                                this
                            ) to kProperties.size
                        }
    ...
    private fun createKProperty(
        ...
        return irBuilder.irBlock(expression) {
            ...
            val getterCallableReference = expression.getter?.owner?.let { getter ->
                ...
                val getterKFunctionType = this@PropertyDelegationLowering.context.irBuiltIns.getKFunctionType(
                    returnType,
                    receiverTypes
                )
            ...
            val setterCallableReference = expression.setter?.owner?.let { setter ->
                if (!isKMutablePropertyType(expression.type)) null
                    ...
                    val setterKFunctionType = this@PropertyDelegationLowering.context.irBuiltIns.getKFunctionType(
